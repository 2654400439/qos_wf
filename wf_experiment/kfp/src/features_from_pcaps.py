...
def defend_zerodelay_alsohidetotalsizeandtime(trace, n_dummies=1.0):
    ...
    if sizeOut % padsize != 0:
        cost_bw += abs(padsize - (sizeOut % padsize))
        newPackets = addPackets(trace2[-1][1], trace2[-1][1] + 60, abs(padsize - (sizeOut % padsize)), 1400,
                                1)  # between [end, end+10s]
        trace2.extend(newPackets)

    if sizeInc % padsize != 0:
        cost_bw += abs(padsize - (abs(sizeInc) % padsize))
        newPackets = addPackets(trace2[-1][1], trace2[-1][1] + 60, abs(padsize - (sizeInc % padsize)), 1400,
                                -1)  # between [end, end+10s]
        trace2.extend(newPackets)
...
def defend_zerodelay_alsohidetotalsize(trace, n_dummies=1.0):
    ...
    if sizeOut % padsize != 0:
        cost_bw += abs(padsize - (sizeOut % padsize))
        newPackets = addPackets(trace2[-1][1], trace2[-1][1], abs(padsize - (sizeOut % padsize)), 1400,
                                1)  # between [end, end+10s]
        trace2.extend(newPackets)

    if sizeInc % padsize != 0:
        cost_bw += abs(padsize - (abs(sizeInc) % padsize))
        newPackets = addPackets(trace2[-1][1], trace2[-1][1], abs(padsize - (sizeInc % padsize)), 1400,
                                -1)  # between [end, end+10s]
        trace2.extend(newPackets)
