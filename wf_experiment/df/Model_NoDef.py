...
# ---------------------------------------------------------
# 必须在 import tensorflow 之前执行以下配置
# ---------------------------------------------------------

# 1. 尝试自动找到 pip 安装的 nvidia-cuda-nvcc 路径
try:
    ...
    # 获取 cudnn 的安装路径 (为了 LD_LIBRARY_PATH)
    cudnn_path = os.path.dirname(nvidia.cudnn.__file__)

    # 手动拼接 bin 目录并加入系统 PATH
    # 这样 TensorFlow 调用 ptxas 时就会先找到这里的，而不是系统的旧版
    ...
    print("❌ 未找到 nvidia-cuda-nvcc，请确保运行了: pip install nvidia-cuda-nvcc-cu11==11.8.89")
