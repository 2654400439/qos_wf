...
def load_and_process_data():
    ...
    # 修改点开始：加载新的 CSV 数据
    # =======================================================
    csv_path = "../preprocessed_data/domain_derived_cv_metrics.csv"
    ...
    try:
        ...
        # 2. 格式对齐：CSV中的域名是 'mzstatic_com'，而 NPZ 通常是 'mzstatic.com'   不需要做
        ...
        metric_cols = ['LCP_TotalByte', 'LCP_RequestCount', 'TTFB_LCP', 'resource_entropy', 'request_size_entropy',
                       'request_timing_entropy', 'http_protocol_cv', 'tcp_cv', 'load_cv', 'js_cv', 'paint_cv', 'style_cv']
...
def analyze_and_visualize(df, metric_cols):
    ...
    # 4. 打印 Top 结果
    print("\n=== Top 正相关指标 (基于 F1_Max) ===")
    print(df_corr[df_corr["Target"] == "F1_Mean"].sort_values("Correlation", ascending=False).head(12))
...
def show_mutual_information(df, cols):
    ...
    mi_scores = mutual_info_regression(df[cols], df["F1_Mean"], random_state=42, n_neighbors=3)
